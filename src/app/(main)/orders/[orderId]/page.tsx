...
import { Button, Col, Image, Modal, Table, TableProps, Tag } from 'antd'
    ...
    <div>
      ...
      <div className="p-16 flex gap-4 float-end">
        {user?.role === UserRole.ORDER_TAKER && (
          <>
            <Button disabled={data?.status !== OrderStatus.CREATED} size="large" danger>
              Cancel order
            </Button>
            <Button
              disabled={data?.payment?.status === PaymentStatus.COMPLETED}
        ...
        {user?.role === UserRole.BARISTA && (
          <>
            {data?.status === OrderStatus.CREATED && (
              <Button onClick={() => setProcessModal(true)} size="large" type="primary">
                Process order
              </Button>
            )}
            {data?.status === OrderStatus.PROCESSING && (
              <Button onClick={() => setCompleteModal(true)} size="large" type="primary">
                Complete order
              </Button>
            )}
          </>

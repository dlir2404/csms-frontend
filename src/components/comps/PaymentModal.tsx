...
import { Button, Input, InputNumber, Menu, MenuProps, Modal } from 'antd'
    ...
    return (
        <>
                ...
                <div className='flex gap-4 h-[400px]'>
                    ...
                    <div className='bg-slate-100 rounded-md flex-1 py-16 px-20'>
                        <div className='text-lg flex flex-col gap-6'>
                            ...
                            <div className={`flex items-center justify-between relative ${!discountValid ? 'mb-4' : ''}`}>
                                <div>Discount</div>
                                <div className=''>
                                    <InputNumber value={discount} onChange={(value) => {
                                        console.log(value)
                                ...
                                {!discountValid && (
                                    <div className='absolute text-sm text-red-500 -bottom-6 right-8'>Discount invalid</div>
                                )}
                            </div>
                            <div className='flex items-center justify-between'>
                                <div>Total</div>
                            ...
                            {paymentMethod === 'cash' && (
                                <div className='flex gap-4'>
                                    ...
                                    <Button className='flex-1' disabled={!discountValid} loading={loading} onClick={showConfirm} size='large' type='primary'>Confirm payment</Button>
                                </div>
                            )}
                            {paymentMethod === 'vnpay' && (
                                <div className='flex gap-4'>
                                    <PrintBill order={order} />
                                    <Button className='flex-1' disabled={!discountValid} loading={loading} onClick={showVNPayQR} size='large' type='primary'>Show payment QR</Button>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            >
                ...
                <div className='text-center mt-4'>or click: <Link target='_blank' href={payUrl}>Link</Link></div>
            </Modal>
        </>
    )
}